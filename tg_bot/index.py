...
def handler(event, context):
    ...
    if "text" in message:
        ...
        if text == "/start":
            params["text"] = ("Привет, я умею:\n/getface - отправлять фото лица, которое не определено в базе данных\n Чтобы сохранить имя для лица, ответь на отправленное фото\n/find <name> - "
                              "отправлять фотографии по отправленному имени ")
        elif text == "/getface":
            r = pool.retry_operation_sync(select_face_witout_name)
            if len(r[0].rows) == 0:
                params["text"] = "Нет неопределенных имен"
            else:
                action = "sendPhoto"
                params[
